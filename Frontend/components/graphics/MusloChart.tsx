...
interface MusloChartProps {
    metrics: Metrica[];
}

const MusloChart: React.FC<MusloChartProps> = ({ metrics }) => {

    const filteredMetrics = metrics.filter(
        m => m.muslo !== undefined && m.muslo !== null && !isNaN(m.muslo)
    );
    ...
    const data = filteredMetrics.map(m => m.muslo as number);
    ...
    return (
        <View style={styles.chartCard}>
            <Text style={styles.chartTitle}> Muslo</Text>
            <LineChart
                data={{
