...
interface PechoChartProps {
    metrics: Metrica[];
}

const PechoChart: React.FC<PechoChartProps> = ({ metrics }) => {

    const filteredMetrics = metrics.filter(
        m => m.pecho !== undefined && m.pecho !== null && !isNaN(m.pecho)
    );
    ...
    const data = filteredMetrics.map(m => m.pecho as number);
    ...
    return (
        <View style={styles.chartCard}>
            <Text style={styles.chartTitle}> Pecho</Text>
            <LineChart
                data={{
